...
class AES(object):
    ...
    def aesInverseRound(self, state, roundKey):
    ...
    def aesMain(self, state, expandedKey, numberOfRounds):
        ...
        state = self.addRoundKey(state,
                                 self.createRoundKey(expandedKey, 16*numberOfRounds))
        return state

    # Perform the initial operations, the standard round, and the final
    # operations of the inverse aes, creating a round key for each round
    def aesInverseMain(self, state, expandedKey, numberOfRounds):
        state = self.addRoundKey(state,
                                 self.createRoundKey(expandedKey, 16*numberOfRounds))
        i = numberOfRounds - 1
        while i > 0:
            state = self.aesInverseRound(state,
                                      self.createRoundKey(expandedKey, 16*i))
            i -= 1
        state = self.shiftRows(state, True)
        state = self.subBytes(state, True)
    ...
    def encrypt(self, input, key, size):
        ...
        if size == self.keySize["SIZE_128"]: numberOfRounds = 10
        elif size == self.keySize["SIZE_192"]: numberOfRounds = 12
        elif size == self.keySize["SIZE_256"]: numberOfRounds = 14
        else: return None
        ...
        block = self.aesMain(block, expandedKey, numberOfRounds)
    ...
    def decrypt(self, input, key, size):
        ...
        block = self.aesInverseMain(block, expandedKey, numberOfRounds)
        # unmap the block again into the output
        for k in range(4):
            # iterate over the rows
            for l in range(4):
...
def testStr(cleartext, keysize=16, modeName = "OFB"):
    #Test with random key, choice of mode.'''
    print 'Rastgele anahtar testi', 'mod:', modeName
    ...
    print 'Sifreleyici:', [ord(x) for x in cipher]
    decr = decryptData(key, cipher, mode)
    print 'Desifreleyici:', decr
    
    
if __name__ == "__main__":
    moo = AESModeOfOperation()
    cleartext = "test icin yazilmis sifrelenecek metin,2. deneme"
